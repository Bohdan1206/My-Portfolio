# Bohdan Romanchuk — Front-End Developer

Hi! I’m Bohdan, a Front-End Developer focused on building clean, functional, and responsive web interfaces.  
I enjoy turning designs into structured, user-friendly web pages and continuously improving my skills.

---

## 🛠 Tech Stack

- HTML5 & CSS3 (semantic markup)
- Responsive Design (Flexbox, Grid, media queries)
- Basic JavaScript (DOM, events, core logic)
- React (learning and practicing)
- Cross-browser compatibility
- Git & GitHub
- Working with Figma / PSD layouts

---

## 📂 Projects

Here are some of my projects hosted on **GitHub Pages**:

- [SaltFrame](https://bohdan1206.github.io/My-Portfolio/SaltFrame/)
- [BeautyZone](https://bohdan1206.github.io/My-Portfolio/BeautyZone/)
- [GGBand](https://bohdan1206.github.io/My-Portfolio/GGBand/)
- [Graphic-Designer-CV](https://bohdan1206.github.io/My-Portfolio/Graphic-Designer-CV/)
- [MyBakery](https://bohdan1206.github.io/My-Portfolio/MyBakery/)
- [MyCV](https://bohdan1206.github.io/My-Portfolio/MyCV/)
- [tradeBladeReact](https://bohdan1206.github.io/My-Portfolio/tradeBladeReact/)

---

## 🚀 About Me

- Entry-level Front-End Developer (Junior / Trainee)
- Open to real projects and collaboration
- Focused on growth and best practices
